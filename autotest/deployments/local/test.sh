...
test_example() {
    ...
    local log_file="${dir}/local_deployment_test.log"
    local test_failed=0
    ...
    echo "Local Deployment Test for $project_name - $(date)" > "$log_file"
    echo "=================================================" >> "$log_file"
    
    log "INFO" "Starting local deployment test for $project_name" "$log_file"
    ...
    echo "Running: preswald deploy --target local"
    if output=$(preswald deploy --target local 2>&1); then
        end_time=$(date +%s)
        ...
        if echo "$output" | grep -i "error\|exception\|failed" > /dev/null; then
            ...
            LOCAL_URL=$(echo "$output" | grep -o 'http://localhost:[0-9]*' | head -1)
            
            if [ ! -z "$LOCAL_URL" ]; then
                log "INFO" "Local URL: $LOCAL_URL" "$log_file"
                
                log "INFO" "Testing deployed application..." "$log_file"
                start_time=$(date +%s)
                sleep 5
                if curl_output=$(curl -s -f "$LOCAL_URL" 2>&1); then
                    end_time=$(date +%s)
                ...
                log "ERROR" "Could not extract local URL from output" "$log_file"
        ...
        log "INFO" "Stopping local deployment..." "$log_file"
        start_time=$(date +%s)
        
        if stop_output=$(preswald stop --target local 2>&1); then
            end_time=$(date +%s)
...
echo "Starting local deployment tests for all example projects"
echo "================================================================="
...
if [ $failed_tests -eq 0 ]; then
    ...
    find examples -maxdepth 2 -name "local_deployment_test.log" ! -path "*/\.preswald_deploy/*" -exec echo "- {}" \;
    exit 1
fi