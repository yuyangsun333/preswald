...
minesDf = minesDf[(minesDf['LAT'] != 0) & (minesDf['LONG'] != 0)] # Filter entries with invalid cords.

# Get and process dataset for deaths funded by diamonds
deathsDf = get_df('deaths_csv') 
deathsDf['War-related deaths'] = deathsDf['War-related deaths'].str.replace(',', '').astype(float) # format numbers
deathsDf = deathsDf[(deathsDf['War-related deaths'] > 5000)] # Remove small data points
deathsDf = deathsDf[deathsDf['Major rebel funding sources'].str.contains('diamonds', case=False, na=False)] # Retrieve only datapoints where military group's funding source includes diamonds
deathsDf['Conflict'] = deathsDf['Country'] + ' (' + deathsDf['Years'] + ')' # Create readable conflict names
...
selected_country = selectbox(
    ...
    options=list(country_zoom.keys()),
    default='All'
)
...
text('The data shows mine clusters in West Africa, where millions were killed by military groups funded by those same gems. TraxNYC argues that direct sourcing helps miners, not militias. But does it?')
...
fig = px.bar(
    ...
    labels={'War-related deaths': 'War-Related Deaths', 'Conflict': 'Conflict (Country & Years)'},
    text_auto=True
)
...
text('TraxNYC claims that "ethical" business practices can end years of exploitation, but the data suggests a much larger problem. The history suggests that the rocks extracted from these mines have helped raise armies, not people out of poverty.')
