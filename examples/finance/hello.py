from preswald import text, plotly, slider
...
fig2 = px.imshow(
    ...
    labels=dict(
        x="Stock", 
        y="Stock", 
        color="Volatility Difference (stdev)"
    ), 
...
df_log_volatility = pd.DataFrame(
    log_volatility_diff, 
    index=tickers, 
    columns=tickers
)
...
fig4 = px.imshow(
    ...
    labels=dict(
        x="Stock", 
        y="Stock", 
        color="Log Volatility Difference (stdev)"
    ), 
...
text("As you can see, the volatility relationships are more or less preserved, but the log returns are in a much closer range of one another in comparison to the raw price data.")
...
if rolling_window > len(returns):
    raise ValueError(f"Rolling window size ({rolling_window}) cannot be larger than the number of rows in the returns DataFrame ({len(returns)}).")
